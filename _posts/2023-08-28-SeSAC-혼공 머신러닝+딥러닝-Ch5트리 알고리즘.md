---
layout: post
title: "[SeSAC]혼공 머신러닝+딥러닝 Chapter5. 트리 알고리즘"
...
- **n_estimators**: 앙상블에 사용할 결정 트리의 수 (디폴트: 100)
- **max_depth**: 각 트리의 최대 깊이 (디폴트: None, 가능한 값: 정수)
- **random_state**: 재현 가능성을 위한 랜덤 시드 (디폴트: None)

**엑스트라트리 (Extra Trees)**
- **n_estimators**: 생성할 트리의 수 (디폴트: 100)
- **max_features**: 각 트리에서 분할에 사용할 특성의 최대 수 (디폴트: 'auto', 가능한 값: 'auto', 'sqrt', 'log2')
- **bootstrap**: 무작위로 데이터를 샘플링할 것인지 여부 (디폴트: False)

**그레이디언트 부스팅 (Gradient Boosting)**
- **learning_rate**: 학습률 (디폴트: 0.1)
- **n_estimators**: 부스팅 단계의 수 (디폴트: 100)
- **subsample**: 각 부스팅 단계에서 사용할 훈련 샘플의 비율 (디폴트: 1.0, 가능한 값: 0~1 사이의 실수)

**히스토그램 기반 부스팅 (Histogram-based Boosting)**
- **learning_rate**: 학습률 (디폴트: 0.1)
- **max_iter**: 최대 반복 횟수 (디폴트: 100)
- **max_leaf_nodes**: 각 트리에서의 최대 리프 노드 수 (디폴트: 31)

**XGBoost**
- **eta**: 학습률 (디폴트: 0.3)
- **max_depth**: 트리의 최대 깊이 (디폴트: 6)
- **subsample**: 훈련 데이터의 하위 샘플 비율 (디폴트: 1)

**LightGBM**
- **learning_rate**: 학습률 (디폴트: 0.1)
- **num_leaves**: 하나의 트리가 가질 수 있는 최대 리프의 수 (디폴트: 31)
- **max_depth**: 트리의 최대 깊이 (디폴트: -1, 의미: 제한 없음)